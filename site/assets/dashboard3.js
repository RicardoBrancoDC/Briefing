...
const SMS_JSON_URL = 'sms_brasil_estoque_atual_por_municipio.json';
let smsBase = null;
let smsMunicipios = {};
let smsIndiceNome = {};
let smsTotaisUF = {};
let smsBaseCarregada = false;
...
document.addEventListener('DOMContentLoaded',()=>{carregarBaseSMS().finally(loadDashboard); timer=setInterval(tick,1000); setInterval(loadDashboard,AUTO_REFRESH_MS)});
...
async function carregarBaseSMS(){
  try{
    const res = await fetch(`${SMS_JSON_URL}?_=${Date.now()}`,{cache:'no-store'});
    if(!res.ok) throw new Error(`HTTP ${res.status}`);
    smsBase = await res.json();
    smsMunicipios = smsBase.municipios || {};
    smsIndiceNome = smsBase.indice_nome || {};
    smsTotaisUF = {};

    Object.values(smsMunicipios).forEach(m=>{
      const uf=String(m.uf||'').toUpperCase();
      const total=Number(m.terminais_aptos_sms||0);
      if(uf) smsTotaisUF[uf]=(smsTotaisUF[uf]||0)+total;
    });

    smsBaseCarregada = true;
  }catch(e){
    console.warn('Não foi possível carregar o JSON nacional de estoque SMS:', e);
    smsBaseCarregada = false;
  }
}
...
function formatSmsNumber(v){
  if(v===null || v===undefined || Number.isNaN(Number(v))) return '--';
  return Number(v).toLocaleString('pt-BR');
}

function estadoNomePorUf(uf){
  return UF_NOME[String(uf||'').toUpperCase()] || '';
}

function extrairCodigosSMS(obj, out=[]){
  if(!obj || typeof obj !== 'object') return out;

  ['cd_mun','codigo_ibge','cod_ibge','geocode','geocodigo','ibge','municipio_codigo','codigoMunicipio'].forEach(k=>{
    const v=obj[k];
    if(v===null || v===undefined) return;
    if(Array.isArray(v)) v.forEach(item=>extrairCodigosSMS({valor:item},out));
    else if(typeof v === 'object') out.push(v.value || v.Value || v.codigo || v.geocode || v.cd_mun || '');
    else out.push(v);
  });

  if(obj.valor) out.push(obj.valor);
  if(Array.isArray(obj.info)) obj.info.forEach(x=>extrairCodigosSMS(x,out));
  if(Array.isArray(obj.area)) obj.area.forEach(x=>extrairCodigosSMS(x,out));
  return out;
}

function areaDoAlertaSMS(a){
  return String(a?.location || a?.areaDesc || a?.area_desc || a?.area || a?.municipio || a?.nm_mun || a?.cidade || a?.uf || '').trim();
}

function calcularSMSPorArea(areaTexto){
  if(!smsBaseCarregada) return {total:null,tipo:'base'};

  const area = String(areaTexto||'').replace(/\s+/g,' ').trim();
  if(!area) return {total:null,tipo:'unknown'};
  ...
  candidatos.forEach(c=>{
    const m = c.match(/^(.+?)\s*\/\s*([A-Z]{2})$/i) || c.match(/^(.+?)\s*-\s*([A-Z]{2})$/i) || c.match(/^(.+?)\s*\(([A-Z]{2})\)$/i);
    if(!m) return;

    const nome = normalizeSearchText(m[1]);
    const uf = normalizeSearchText(m[2]);
    if(!uf || uf.length!==2) return;

    const nomeEstado = normalizeSearchText(estadoNomePorUf(uf));
    if(nomeEstado && nome === nomeEstado && smsTotaisUF[uf] !== undefined){
      const key = `UF-${uf}`;
      if(!usados.has(key)){
        usados.add(key);
        total += Number(smsTotaisUF[uf]||0);
        achou++;
        tipo = 'uf';
      }
      return;
    ...
    const chave = `${nome}-${uf}`;
    const cd = smsIndiceNome[chave];
    if(cd && smsMunicipios[cd] && !usados.has(cd)){
      usados.add(cd);
      total += Number(smsMunicipios[cd].terminais_aptos_sms||0);
      achou++;
    }
  });

  if(!achou) return {total:null,tipo:'unknown'};
  return {total,tipo,quantidade:achou};
}

function calcularSMSPorAlerta(a){
  if(!smsBaseCarregada) return {total:null,tipo:'base'};

  const codigos = extrairCodigosSMS(a,[])
  ...
  if(codigos.length){
    ...
    let total = 0;
    codigos.forEach(cd=>{
      if(smsMunicipios[cd] && !usados.has(cd)){
        usados.add(cd);
        total += Number(smsMunicipios[cd].terminais_aptos_sms||0);
      }
    });
    if(usados.size) return {total,tipo:'ibge',quantidade:usados.size};
  }

  return calcularSMSPorArea(areaDoAlertaSMS(a));
}

function atualizarSMS24h(alerts){
  const el = byId('sms-total-24h');
  const sub = byId('sms-total-sub');
  if(!el) return;

  if(!smsBaseCarregada){
    el.textContent = '--';
    if(sub) sub.textContent = 'Aguardando JSON nacional de estoque SMS';
    return;
  }
  ...
  el.textContent = formatSmsNumber(total);
  if(sub) sub.textContent = `Soma estimada de ${contados} alertas das últimas 24h`;
}
...
function renderAll(data){const alerts=(data.all_alerts||data.latest_alerts||[]).map(normalizeAlert); const latest=(data.latest_alerts||alerts).map(normalizeAlert); const total=alerts.length || num(data.summary?.total_alerts)||num(data.cards?.ultimas24h)||0; const vigentes=alerts.filter(a=>isVigente(a)).length || num(data.cards?.vigentes)||0; const levels=aggregate(alerts,'nivel'); const ufAgg=aggregate(alerts,'uf'); const topUfs=Object.entries(ufAgg)
  ...
  renderLevelDonut(levels,total); renderCategory(alerts,data); atualizarSMS24h(alerts); renderHourly(alerts,last.sent||data.generated_at); renderEmitters(data.top_emitters||[],alerts); renderEvents(data.event_distribution||[],alerts); renderActiveLevels(alerts); renderStates(topUfs,total); renderRegion(alerts); renderSince(alerts); renderTable(latest.length?latest:alerts); renderMap(topUfs,alerts);
}
